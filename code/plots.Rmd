...
affr_dft_l <- readRDS("./data/dataframes/affr_dft_l.rds") %>%
  mutate(
    ipa = recode(labels, tS = "tʃ", dZ = "dʒ", c = "t̻ʃ̻", J = "d̻ʒ")
  )
...
moments <- bind_rows(fric_moments, affr_moments) %>%
  mutate(
    ipa = factor(ipa,
      levels = c("f", "θ", "s", "ʃ", "v", "ð", "z", "ʒ", "h", "ts", "tʃ", "t̻ʃ̻", "dz", "dʒ", "d̻ʒ")
    ),
    ...
    poa = case_when(
      ...
      ipa %in% c("t̻ʃ̻", "d̻ʒ") ~ "lamino-postalveolar",
      ipa %in% c("h") ~ "glottal"
    ),
...
ggsave("./img/int-all-plot.png", width = 14, height = 10)
```
...
ggsave("./img/int-decl-plot.png", width = 10, height = 14)
```
...
ggsave("./img/vowels-all.png", width = 7, height = 5)
```
