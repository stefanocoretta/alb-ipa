...
if (!dir.exists(seglist_out)) {
 s04_words <- query(
  ...
  words <- unique(seglist$word)
  wavs <- list.files(seglist_out, "*.wav")
  
  file.rename(paste0(seglist_out, "/", wavs), paste0(seglist_out, "/", seglist$file_name))
  
  sil <- silence(0.8, xunit = "time", bit = 16, pcm = T)
  
  dir.create(str_glue("{seglist_out}/words"), showWarnings = F)
  ...
  for (word in words) {
    ...
    combined <- bind(sil, firstw, sil, secondw, sil, thirdw, sil)
    writeWave(combined, str_glue("{seglist_out}/words/{word}.wav"))
  }
}
```
