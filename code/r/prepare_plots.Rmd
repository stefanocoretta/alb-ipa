...
uti_vowels_2 <- readRDS("./data/dataframes/uti-vowels-2.rds")
```
...
ggsave("./img/emu/vowels-z.png", width = 7, height = 5)
```
...
ggsave("./img/emu/vowels-z-hz.png", width = 7, height = 5)
```
...
ggsave("./img/emu/vowels-z-hz-ipa.png", width = 7, height = 5)
```
...
ggsave("./img/emu/vowels-z-hz-bark.png", width = 7, height = 5)
```
...
ggsave("./img/emu/vowels-z-hz-log.png", width = 7, height = 5)
```
...
ggsave("./img/emu/intonation.png", width = 7, height = 5)
```
...
ggsave("./img/emu/intonation-all.png", width = 7, height = 5)
```
...
ggsave("./img/emu/uti-laterals-all.png", width = 7, height = 5)
```
...
ggsave("./img/emu/uti-vowels-gam.png", width = 7, height = 5)
```

```{r vowels-gam-2}
vowels_gam_2 <- polar_gam(
  Y ~
    s(X, prompt, bs = "fs", k = 8),
  data = uti_vowels_2 %>% mutate(prompt = as.factor(prompt)),
  fan_lines = c(15, 20)
)
```

```{r vowels-gam-2-plot}
predict_polar_gam(vowels_gam_2, length_out = 100) %>%
  ggplot(aes(X, Y, colour = prompt)) +
  geom_path(size = 2, alpha = 0.8) +
  annotate(
    "label",
    x = c(10, 15, -5, 30, -4, 1, 35),
...
ggsave("./img/emu/uti-vowels-gam-2.png", width = 7, height = 5)
```
