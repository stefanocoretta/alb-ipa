...
uti_vows <- readRDS("./data/dataframes/uti-vows.rds")
```
...
vows_gam <- polar_gam(
  Y ~
    s(X, prompt, bs = "fs", k = 8),
  data = uti_vows %>% mutate(prompt = as.factor(prompt)),
  fan_lines = c(15, 20)
)
```

```{r}
predict_polar_gam(vows_gam, length_out = 100) %>%
  ggplot(aes(Y, X, colour = prompt)) +
  geom_path(size = 2, alpha = 0.8) +
  annotate(
    "label",
    x = c(10, 10, -5, 30, -4, 1, 35),
    y = c(9, 24, 5, 20, 15, 20, 7),
    label = c("a", "e", "ɜ", "i", "ɔ", "u", "y"),
    fill = c(get_pal("Hoiho"), "#490B0A"),
    colour = c("white", "white", "black", "white", "white", "white", "white"),
    size = 6,
