...
palate <- rticulate::read_aaa("./data/ultrasound/palate-spline.txt", column_names = c("rec_date", "time", "prompt"))
```
...
vot %>%
  ...
  geom_jitter(width = 0.25, size = 3.5, alpha = 0.8) +
...
ggsave(here(arti_dir, "uti-laterals-all.png"), width = 7, height = 5)
```
...
```{r vowels-2}
uti_vowels_2 %>%
  ggplot(aes(X, Y, group = .index)) +
  geom_path(aes(colour = prompt, linetype = prompt)) +
  geom_path(data = palate %>% filter(fan_line < 20)) +
  labs(
    colour = "vowel", linetype = "vowel",
    x = "back-to-front (mm)", y = "low-to-high (mm)"
  )
...
```{r vowels-gam-2-plot}
v_gams_2_preds <- predict_polar_gam(vowels_gam_2, length_out = 100)

v_highest_2 <- v_gams_2_preds %>%
  group_by(prompt) %>%
  filter(Y == max(Y)) %>%
...
ggsave(here(arti_dir, "uti-vowels-gam-2.png"), width = 7, height = 5)
```
